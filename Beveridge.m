...
X = fsolve(@DMP_solve,X00,Options,b,p,r,k,s,beta,alpha0,alpha1); %fsolve function
...
figure(1)
plot(bc_space,u_space,...
    'LineWidth',2,...
    'color',[0 0 1]); hold on, 
plot(jc_space,u_space,...
    'LineWidth',2,...
    'color',[0.9 0.2 .4])
ylabel('v')
xlabel('u')
...
X00 = [0.1,0.5,0.5]; % initial w, v and u values for fsolve
X = fsolve(@DMP_solve_alt,X00,Options,phi,p,r,k,s,beta,alpha0,alpha1); %fsolve function

w = X(1);
v = X(2);

    theta = 3; % this theta value is obtained from the equilibrium from the other model!
    M = alpha0*u^(alpha1)*v^(1-alpha1);   % matching function
    q = M/v;                              % job filling rate
    f = theta*q;                          % job finding rate
